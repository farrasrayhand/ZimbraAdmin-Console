...
class ZimbraAdmin {
  ...
  async searchAccounts(query, limit = 50, offset = 0, domain = '') {
    ...
    if (q && !q.includes('=')) {
      ...
      if (clean) {
        q = `(|(mail=*${clean}*)(zimbraMailAlias=*${clean}*)(zimbraMailDeliveryAddress=*${clean}*)(cn=*${clean}*)(displayName=*${clean}*)(sn=*${clean}*))`;
      } else {
        q = 'cn=*';
      }
    }
    ...
    return {
      accounts: accounts.map(a => {
        ...
        // Extract aliases for this account
        let aliases = [];
        if (a.alias) {
          aliases = Array.isArray(a.alias) ? a.alias : [a.alias];
        } else if (Array.isArray(a.a)) {
          aliases = a.a
            .filter(attr => attr && attr.n === 'zimbraMailAlias')
            .map(attr => attr._ !== undefined ? attr._ : (attr.value !== undefined ? attr.value : ''))
            .filter(Boolean);
        }
        if (aliases.length === 0 && attrs.zimbraMailAlias) {
          aliases = [attrs.zimbraMailAlias];
        }
  ...
  async delegateAuth(accountName) {
    const isId = !accountName.includes('@') && /^[0-9a-fA-F-]{20,}$/.test(accountName.trim());
    const accountByAttr = isId ? 'by="id"' : 'by="name"';
    const body = '<DelegateAuthRequest xmlns="urn:zimbraAdmin" duration="3600">'
      + '<account ' + accountByAttr + '>' + escapeXml(accountName) + '</account>'
      + '</DelegateAuthRequest>';

    const bodyEl = await this._request(body, true);
    const resp = findResponse(bodyEl, 'delegateauthresponse');

    if (!resp) {
      throw { reason: 'Gagal melakukan login sebagai user: tidak ada DelegateAuthResponse dari server Zimbra' };
    }

    const rawToken = pick(resp, ['authToken', '{urn:zimbraAdmin}authToken']);
    const authToken = typeof rawToken === 'string' ? rawToken : rawToken?._;
    const rawLifetime = pick(resp, ['lifetime', '{urn:zimbraAdmin}lifetime']);
    const lifetime = typeof rawLifetime === 'string' ? rawLifetime : (rawLifetime?._ || rawLifetime);

    if (!authToken) {
      throw { reason: 'Gagal mendapatkan auth token delegasi untuk akun ' + accountName };
    }

    // Attempt to discover publicMailURL for the user's mailbox
    let publicMailURL = '';
    try {
      const infoBody = '<GetAccountInfoRequest xmlns="urn:zimbraAdmin">'
        + '<account ' + accountByAttr + '>' + escapeXml(accountName) + '</account>'
        + '</GetAccountInfoRequest>';
      const infoEl = await this._request(infoBody, true);
      const infoResp = findResponse(infoEl, 'getaccountinforesponse');
      if (infoResp) {
        const rawUrl = pick(infoResp, ['publicMailURL', '{urn:zimbraAdmin}publicMailURL']);
        publicMailURL = typeof rawUrl === 'string' ? rawUrl : (rawUrl?._ || '');
      }
    } catch (e) {
      // Ignored: fallback to admin URL host
    }

    // Determine target webmail URL
    let baseWebmail = process.env.ZIMBRA_WEBMAIL_URL ? process.env.ZIMBRA_WEBMAIL_URL.trim() : '';
    if (!baseWebmail && publicMailURL && publicMailURL.trim()) {
      baseWebmail = publicMailURL.trim();
    }

    let targetHostUrl = '';
    try {
      const adminParsed = new URL(this.adminUrl);
      if (baseWebmail) {
        const webParsed = new URL(baseWebmail);
        // If Zimbra returns internal port 7070 or 8080 (backend mailbox port),
        // but admin connection is over HTTPS, rewrite to standard HTTPS without internal port
        if (['7070', '8080'].includes(webParsed.port) && adminParsed.protocol === 'https:') {
          webParsed.protocol = 'https:';
          webParsed.port = '';
        }
        // If publicMailURL returned localhost/127.0.0.1 while admin is accessed via an external domain/IP
        if ((webParsed.hostname === 'localhost' || webParsed.hostname === '127.0.0.1') &&
            adminParsed.hostname !== 'localhost' && adminParsed.hostname !== '127.0.0.1') {
          webParsed.hostname = adminParsed.hostname;
        }
        targetHostUrl = webParsed.origin;
      } else {
        // Fallback from admin URL: same protocol and host, standard webmail port (omitting 7071)
        const protocol = adminParsed.protocol || 'https:';
        const hostname = adminParsed.hostname;
        targetHostUrl = `${protocol}//${hostname}`;
      }
    } catch {
      const domainPart = accountName.includes('@') ? accountName.split('@')[1] : 'localhost';
      targetHostUrl = `https://${domainPart}`;
    }

    const cleanBase = targetHostUrl.replace(/\/+$/, '');
    const loginUrl = `${cleanBase}/service/preauth?authtoken=${encodeURIComponent(authToken)}&isredirect=1&adminPreAuth=1`;
